...
import { TicketFormSchema, TicketWithTags, getSubAccountTeamMembers } from '@/lib/types'
import { useModal } from '@/providers/modal-provider'
import { zodResolver } from '@hookform/resolvers/zod'
import { Tenant, Tag, User } from '@prisma/client'
...
const TicketForm = ({ getNewTicket, laneId, id }: Props) => {
  const { data: defaultData, setClose } = useModal()
  const router = useRouter()
  const [tags, setTags] = useState<Tag[]>([])
  const [contact, setContact] = useState('')
  const [search, setSearch] = useState('')
  const [contactList, setContactList] = useState<Tenant[]>([])
  const saveTimerRef = useRef<ReturnType<typeof setTimeout>>()
  const [allTeamMembers, setAllTeamMembers] = useState<User[]>([])
  const [assignedTo, setAssignedTo] = useState(
    defaultData.ticket?.AssignedsysUser?.id || ''
  )
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        const response = await getSubAccountTeamMembers(id)
  ...
  useEffect(() => {
    if (defaultData.ticket) {
      ...
      if (defaultData.ticket.tenantId)
        setContact(defaultData.ticket.tenantId)

      const fetchData = async () => {
        const response = await searchContacts(
          //@ts-ignore
          defaultData.ticket?.Tenant?.name
        )
      ...
      const response = await upsertTicket(
        {
          ...
          AssignedsysUserId: assignedTo,
          ...(contact ? { tenantId: contact } : {}),
        },
        tags
      )
