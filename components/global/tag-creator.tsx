...
const TagCreator = ({ getSelectedTags, sysUserId, defaultTags }: Props) => {
  ...
  useEffect(() => {
    if (sysUserId) {
      const fetchData = async () => {
        const response = await getTagsForSubaccount(sysUserId)
  ...
  const handleAddTag = async () => {
    ...
    setSelectedColor('')
    try {
      const response = await upsertTag(sysUserId, tagData)
      toast({
